...
const fetchData = async () =>{
  ...
  const url = "http://87.71.64.163:5000/getClassesTeacher?teacher="+splits[3]
...
export default function TeacherClasses() {
    ...
    async function openClass() {
      if (className !== null && className.length > 0) {
        ...
        const url = "http://87.71.64.163:5000/openClass?teacher=" + splits[3] + "&className=" + className;
  ...
  async function deleteElement(id){
    ...
    const thisURL = window.location.href;
    const splits = thisURL.split('/')
    const url = "http://87.71.64.163:5000/removeClass?teacher="+splits[3]+"&className=" + classNameDelete
    const promise =  await fetch(url)
    if(promise.status ===200){
