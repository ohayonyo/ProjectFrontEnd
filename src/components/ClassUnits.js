...
export default function ClassUnits() {
  ...
  const [messages, setMessages] = useState([]);
    useEffect(()=>{
      ...
      async function fetchDataCall(){
          const url = "http://87.71.64.163:5000/getClassUnits?teacher="+splits[3]+"&className="+className;
  ...
  const handleSave = (event, index) =>{
    console.log("save")
    const url = "http://87.71.64.163:5000/quickEditUnit?teacher="+splits[3]+"&unitName="+unitNamesAtUpload[index]+
  ...
  async function deleteElement(id) {
    if (messages !== null && messages.length > 0) {
      ...
      const url =
        "http://87.71.64.163:5000/removeUnit?unitName=" +
        unitNameToDelete +
        "&className=" +
        className +
        "&teacher=" +
