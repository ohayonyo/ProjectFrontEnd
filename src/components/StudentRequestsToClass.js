...
const fetchData = async () =>{
  ...
  const url = "http://87.71.64.163:5000/getAllClassesNotIn?username="+splits[3];
...
const fetchData2 = async () =>{
  ...
  const thisURL = window.location.href;
  const splits = thisURL.split('/')
  const url = "http://87.71.64.163:5000/getAllClassesWaiting?username="+splits[3];
  const result = await fetch(url)      
  const jsonResult = await result.json();
  console.log("json result is ")
  console.log(jsonResult)
...
export default function StudentRequestsToClass() {
  ...
  async function signUpToClass(){
    ...
    const url = "http://87.71.64.163:5000/registerClass?student="+ splits[3]+"&className="+ (select ? select : classes[0].className)
    console.log(url)
    ...
    async function rejectStudent(id){
      ...
      const url = "http://87.71.64.163:5000/removeRegistrationClass?student="+ splits[3]+"&className="+ toDelete[0].className
      console.log(url)
      const promise =  await fetch(url)
